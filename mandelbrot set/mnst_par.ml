...
let aux w rows rem t1 t2 cnt ptr =
  (* let mybuf =  (Bytes.create 0) in  *)
  for i = t1 to t2 do
    let red_i = i - 1 in
    let buf = worker w (red_i * rows + min red_i rem) (i * rows + min i rem) in
    cnt.(ptr)<-(   cnt.(ptr) + Bytes.length  buf  );
...
let _ =
  ...
  let f1 = Domain.spawn(fun () -> aux  w rows rem   1     (workers/4)  cnt  0) and
  f2 = Domain.spawn(fun () -> aux w rows rem  (workers/4)   (workers/2)   cnt 1) and
  f3 = Domain.spawn(fun () -> aux w rows rem  (workers/2)  ((3*workers)/4) cnt 2) and
  f4 = Domain.spawn (fun () -> aux  w rows rem  ((3*workers)/4)  workers  cnt 3) in
